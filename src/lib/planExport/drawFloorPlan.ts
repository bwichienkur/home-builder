...
export function buildPlanDxf(input: PlanExportInput): string {
  ...
  // Unit-length block geometry (scaled via INSERT 41/42).
  const winHalf = unit === 'metric' ? 300 : 12;
  ...
  const lines: string[] = [
    ...
    '2',
    'BLOCKS',
    // Unit door leaf: hinge at origin, leaf along +X, swing arc toward +Y.
    '0',
    'BLOCK',
    '8',
    '0',
    ...
    '0',
    'ENDBLK',
    // Unit window: two parallel glazing lines.
    '0',
    'BLOCK',
    '8',
    '0',
  ...
  for (const room of input.planRooms) {
    ...
    // Solid floor hatch for room fill (A-FLOR).
    lines.push(
      '0',
      'HATCH',
      '8',
    ...
    for (let i = 0; i < verts.length; i++) {
      const a = verts[i]!;
      const bPt = verts[(i + 1) % verts.length]!;
      lines.push('72', '1', '10', String(a.x), '20', String(a.y), '11', String(bPt.x), '21', String(bPt.y));
    }
    lines.push('97', '0');
    const cx = room.points.reduce((s, p) => s + p.x, 0) / room.points.length;
    const cy = room.points.reduce((s, p) => s + p.y, 0) / room.points.length;
  ...
  input.openings.forEach((opening, i) => {
    ...
    pushLine(layer, span.a, span.b);
    pushLine('OPENINGS', span.a, span.b);
    const A = scaleCad(span.a);
    const B = scaleCad(span.b);
    const mid = scaleCad(span.mid);
    const ang = (Math.atan2(B.y - A.y, B.x - A.x) * 180) / Math.PI;
    const widthCad = Math.hypot(B.x - A.x, B.y - A.y) || 1;
    if (isDoor) {
      // INSERT DOOR at hinge (span.a), scale X to clear width.
      lines.push(
        '0',
        'INSERT',
        '8',
      ...
      lines.push(
        ...
        String(widthCad),
        '42',
        String(Math.max(winHalf * 0.02, widthCad * 0.12)),
        '43',
        '1',
        '50',
        String(ang),
