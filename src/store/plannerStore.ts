...
type PlannerState = SceneSnapshot & {
 ...
 selectFurniture:(id:string|null)=>void; updateFurniture:(id:string,patch:Partial<FurnitureItem>)=>void; moveSelected:(dx:number,dz:number)=>void; duplicateSelected:()=>void; deleteSelected:()=>void;
...
export const usePlannerStore=create<PlannerState>((set,get)=>{
 ...
 const commit=(next:SceneSnapshot)=>set(s=>{const history=s.history.slice(0,s.historyIndex+1).concat(next);const floors=s.floors.map(f=>f.id===s.activeFloorId?{...f,scene:next}:f);return{...next,floors,history,historyIndex:history.length-1}});
 ...
 return {...initial,tool:'wall',view:'2d',cameraMode:'orbit',selectedWallId:null,selectedFurnitureId:null,draftStart:null,floors:[{id:'ground',name:'Ground floor',scene:initial}],activeFloorId:'ground',history:[initial],historyIndex:0,
  ...
  selectFurniture:(selectedFurnitureId)=>set({selectedFurnitureId,selectedWallId:null}),updateFurniture:(id,patch)=>mutate({furniture:get().furniture.map(f=>f.id===id?{...f,...patch,x:patch.x===undefined?f.x:Math.round(patch.x*4)/4,z:patch.z===undefined?f.z:Math.round(patch.z*4)/4}:f)}),
